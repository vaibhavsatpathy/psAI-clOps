...
def read_text_from_pages(
    ...
    if os.path.isfile(final_path_for_data):
        ...
        file_exists = False
    print("####  Reading pages ####")
    doc_image_types = sorted(os.listdir(complete_folder_path))
    text_of_all_pages = []
    for doc_image_type in doc_image_types:
        if doc_image_type != ".DS_Store":
            print("DOCUMENT TYPE: ", doc_image_type)
            complete_doc_image_path = os.path.join(complete_folder_path, doc_image_type)
            ...
            for page in tqdm(pages):
                if page != ".DS_Store":
                    ...
                    if page_path not in data:
                        ...
                        document_text = pytesseract.image_to_string(document_page)
                        document_page.close()
                        essential_file_path_and_text = (
                            page_path
                            + "####"
                            + preprocess_spacy(document_text, num_of_words=num_of_words)
                            + "\n"
                        )
